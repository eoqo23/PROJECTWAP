...
if (isset($_GET['status']) && isset($_GET['task_id'])) {
    ...
    $new_status = in_array($_GET['status'], ['pending','progress','done']) ? $_GET['status'] : 'pending';
    mysqli_query($koneksi, "UPDATE task_admin SET status='$new_status' WHERE id='$task_id' AND admin_id='$admin_id'");
...
a.status-btn {padding:5px 10px;border-radius:6px;color:white;text-decoration:none;transition:0.3s;margin:0 3px;display:inline-block;}
a.status-btn.pending {background:#facc15;color:black;}
a.status-btn.progress {background:#3b82f6;color:white;}
a.status-btn.done {background:#22c55e;color:white;}
a.status-btn.pending:hover {background:#fbbf24;}
a.status-btn.progress:hover {background:#2563eb;}
                ...
                <tr>
                    ...
                    <td>
                        <?php if($task['status'] !== 'done'): ?>
                            <a class="status-btn progress" href="taskadmin.php?status=progress&task_id=<?= $task['id']; ?>">Sedang</a>
                            <a class="status-btn done" href="taskadmin.php?status=done&task_id=<?= $task['id']; ?>">Selesai</a>
                        <?php else: ?>
                            <span class="status-btn done">Selesai</span>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
            <tr><td colspan="6" style="color:#94a3b8;">Belum ada task untuk user.</td></tr>
        <?php endif; ?>
    </table>
